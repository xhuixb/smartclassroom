...
$query = "select ga45_codi as codiequipament,ga45_descripcio as descequipament,ga45_es_actiu as esactiu,"
        ...
        . " from ga45_inventari order by esactiu desc,totalprestecs asc,ga45_codi asc";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($row['esactiu'] == '1') {
            ...
            $colorEstat = "btn-danger";
            $textEstat = "Fora de préstec";
            $estat = '1';
            $toolTip = "No està disponible per a ser prestat";
        }
        ...
        if ($filtre == "1" || (($filtre == "2") && ($row['codialumne'] == '')) || (($filtre == "3") && ($row['codialumne'] != ''))) {
            ...
            echo '<td class="col-sm-1"><button  type="button" class="btn ' . $colorEstat . ' form-control" onclick="canviaEstatPrestec(this)" data-estat="' . $estat . '" data-tooltip="tooltip" title="' . $toolTip . '" '.$esPrestable.'>' . $textEstat . '</button></td>';
