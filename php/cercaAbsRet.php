...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (($nomesAbsents === '0' && $nomesRetards === '0') ||
            ...
            $numSetmanes = intdiv($diff, 7);

            $diaSetmanaInici = date("w", $dataIniHoresTeor->getTimestamp());
            if ($diaSetmanaInici == 0) {
                $diaSetmanaInici = 7;
            }

            $diaSetmanaFi = date("w", $dataFiHoresTeor->getTimestamp());
            if ($diaSetmanaFi == 0) {
                $diaSetmanaFi = 7;
            }
            //echo 'setmana ini :' . $diaSetmanaInici . " setmana fi: " . $diaSetmanaFi . ' setmanes: ' . $numSetmanes . '<br>';


            $diesResidus = [];
            $residuSetmanes = $diff % 7;

            for ($i = 0; $i < $residuSetmanes; $i++) {
                if (($diaSetmanaInici + $i) <= 7) {
                    $diesResidus[$i] = $diaSetmanaInici + $i;
                } else {
                    $diesResidus[$i] = ($diaSetmanaInici + $i) - 7;
                }
            }

            $diesResidusString = join("-", $diesResidus);
            //echo 'dies residus: ' . $diesResidusString;
            //echo $diff->format('%R%a dies');
            //calculem les sessions teòriques
            //les procedents de grups personals
            $query1 = "select ga26_codi_professor as codiprof,ga26_dia_setmana as dia,count(ga26_dia_setmana) as diaconta"
                    . " from ga26_horaris_docents,ga23_grups_profes_cap,ga24_grups_profes_det"
                    . " where ga26_codi_curs=" . $_SESSION['curs_actual'] . " and ga26_tipus_grup=1 and ga26_grup=ga23_codi_grup and ga26_grup=ga24_codi_grup and ga24_codi_alumne=" . $row['codialumne'] . $whereProfessorHorari . $whereAssignaturaHorari . " and ga26_is_lectiva=1 group by dia order by dia";
            ...
            if ($result1->num_rows > 0) {
                ...
                while ($row1 = $result1->fetch_assoc()) {
                    //contarem els dies que hi ha entre les dues dates
                    $contaFestius = 0;
                    //anem a veure quins festius s'han de restar
                    for ($i = 0; $i < count($diesFestiusArray); $i++) {
                        $festiuTeoric = date_create_from_format("Y-m-d", $diesFestiusArray[$i]);
                        $festiuTeorDiaSetmana = date("w", $festiuTeoric->getTimestamp());

                        if ($festiuTeorDiaSetmana == 0) {
                            $festiuTeorDiaSetmana = 7;
                        }

                        $diff1 = date_diff($dataIniHoresTeor, $festiuTeoric);
                        $diff1 = intval($diff1->format("%R%a"));

                        $diff2 = date_diff($festiuTeoric, $dataFiHoresTeor);
                        $diff2 = intval($diff2->format("%R%a"));
                        //echo 'dif1: ' . $diff1 . ' dif2 ' . $diff2 . ' fesetiu: ' . $festiuTeorDiaSetmana . ' dia: ' . $row1['dia'];
                        if ($diff1 >= 0 && $diff2 >= 0 && $festiuTeorDiaSetmana == $row1['dia']) {
                            //  echo 'true';
                            //restarem les sessions dels festius que s'escaune en el mateix dia de la setmana
                            $contaFestius++;
                        }
                    }

                    //echo '<br>';
                    //echo $contaFestius.'<br>';

                    if (strpos($diesResidusString, $row1['dia']) === false) {

                        //no és un dia resitual
                        $sessionsTeoriques = $sessionsTeoriques + $row1['diaconta'] * ($numSetmanes - $contaFestius);
                    } else {
                        //sí que és residual
                        $sessionsTeoriques = $sessionsTeoriques + $row1['diaconta'] * ($numSetmanes - $contaFestius + 1);
                    }
                }
            }
            ...
            //les procedents de grups generals

            $query1 = "select ga26_codi_professor as codiprof,ga26_dia_setmana as dia,count(ga26_dia_setmana) as diaconta"
                    . " from ga26_horaris_docents,ga12_alumnes_curs"
                    . " where ga26_codi_curs=" . $_SESSION['curs_actual'] . " and ga26_codi_curs=ga12_codi_curs and ga26_tipus_grup=0 and ga26_grup=ga12_codi_grup and ga26_nivell=ga12_codi_nivell and ga12_id_alumne=" . $row['codialumne'] . $whereProfessorHorari . $whereAssignaturaHorari . " and ga26_is_lectiva=1 group by dia order by dia";
    ...
    echo '<td class="col-sm-2">TOTALS</td>';
