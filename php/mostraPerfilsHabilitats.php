...
echo '<table id="taulaPerfilsHabilitats" class="table table-fixed">';
echo '<thead>';
echo '<tr>';
echo '<th class="col-sm-2">Traspàs</th>';
echo '<th class="col-sm-3">Perfils habilitats</th>';
echo '</tr>';
echo '</thead>';
...
if ($result->num_rows > 0) {
    echo '<tbody id="costaulaPerfilsHabilitats">';

    while ($row = $result->fetch_assoc()) {
        if (array_search($row['codiperfil'], $nodeArrayHtml) !== false) {
            echo '<tr>';
            echo '<td class="col-sm-2"><input type="checkbox" class="permisosEnrere"/></td>';
