...
class GestioMails {
    ...
    public function __construct($compteSMTP, $passwordsSMPT, $SMTP) {
        ...
        $this->mail->AddEmbeddedImage('../imatges/logo_Rocagrossa.jpg', 'logo_roca');
    ...
    public function adjuntaFitxer($adjunts) {

        $fitxers = [];

        $fitxers = explode('<#>', $adjunts);

        for ($i = 0; $i < count($fitxers); $i++) {
            $this->mail->addAttachment('../pdf/mailings/'.$fitxers[$i]);
                       
        }
    }

    public function eliminaAdjunts(){
